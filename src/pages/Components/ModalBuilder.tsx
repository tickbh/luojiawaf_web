...
const ModalBuilder = ({
  ...
  const [helpDictTip, setHelpDictTip] = useState<Record<string, string>>({}); // 排序类型
  ...
  const FormlistFun = () => {
    ...
    if (modalKeyList) {
      ...
      for (const key in modalKeyList) {
        ...
        const info1 = autocomplete_key[key] || {};
        if (!(key in helpDictTip)) {
          helpDictTip[key] = info1[modalKeyList[key]];
        }
