...
const Index = () => {
  ...
  const NameList = ['请求类型', '地址', '时间戳', '信息详情'];
  const { Search } = Input;
  const { Option } = Select;
  const values = {
    page: page,
  ...
  // 当页数和展示数量发生改变时,重新发起请求
  const _get_client_attack_visits = async () => {
    if (blockNameList.length == 0) {
      await _getBlockName();
    }
    const value = await get_client_attack_visits({ ...values });
    if (value.success > 0) {
      setTableData(value);
    }
    return value;
