...
const Index = () => {
  ...
  const setTableData = (value: any) => {
    ...
    datas.forEach((element: any, index: any) => {
      ...
      for (const ip in isMap) {
        if (isMap[ip] != 'log') delete actions[0];
        dataSource.push({
          key: index,
          ip: ip || '',
          action: isMap[ip] || '',
  ...
  const _get_record_ip_list = async () => {
    // console.log(values, 9999);
    const value = await get_record_ip_list({ ...values });
  ...
  // 删除
  const _del_record_ip_client = async (_values: any) => {
    const value = await del_record_ip_client({ ..._values });
    console.log(value, '_del_record_ip_client');
    return value;
  };

  // 新增
  const _add_record_ip_client = async (_values: any) => {
    const value = await add_record_ip_client({ ..._values });
    console.log(value, '_add_record_ip_client');
    return value;
  };
  ...
  const handleOk: any = (id: any, type: string) => {
    ...
    const actionFuncType: any = {
      update: _add_record_ip_client,
      delete: _del_record_ip_client,
    };
    console.log(id, type);
  ...
  async function confirm(e: any) {
    ...
    const result = await _del_record_ip_client(_values);
    if (result.success < 0) {
      message.error({
        content: result.err_msg,
        style: {
